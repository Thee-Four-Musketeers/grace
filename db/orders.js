...
async function renderCart(id) {
    try {
        const { rows: product } = await client.query(`
            SELECT o.customer, p.name, op."productId", p.price, op.id, op."orderId", p."imageUrl", p.description, op."productIdQuantity" AS count
            FROM orders AS o
...
async function getOrderByUser(customer) {
    console.log('did it make it to the db method?', customer)
    try {
